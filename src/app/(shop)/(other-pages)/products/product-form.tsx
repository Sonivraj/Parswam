...
export function ProductForm({ product }: { product: TProductItem }) {
  ...
  const { options = [], selected_options = [], collections = [], title = '', price = 0, vendor = '' } = product || {}

  const status = 'in stock'

  // Safe color extraction with proper error handling
  const currentColor = selected_options?.filter((option) => option?.name === 'Color')?.[0]?.value || ''

  // Safe collection access
  const collection = collections?.[0] || { title: 'Products', handle: 'all' }
      ...
      <div>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-4">
          <div className="rounded-full bg-zinc-900 px-5 py-2">
            <Text className="text-xs text-white">{vendor}</Text>
          </div>
          <Text className="text-xl">₹{Number(price).toFixed(2)}</Text>
